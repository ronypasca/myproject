...
{
    public class DUserBudgetPlanAccessDA : BaseDataAccess
    {

        #region Public Property

        public DUserBudgetPlanAccess Data { get; set; }

        #endregion

        #region Constructors
        public DUserBudgetPlanAccessDA()
        {
            Data = new DUserBudgetPlanAccess();
        }
        #endregion
        ...
        {
            PropertyInfo m_pifDUserBudgetPlanAccess = Data.GetType().GetProperty(fieldName);
            return IsPrimaryKey(m_pifDUserBudgetPlanAccess);
        }
        ...
        public override void Insert(bool withTrans, object dbConnection = null)
        {
            ...
            PropertyInfo[] m_pifDUserBudgetPlanAccesses = Data.GetType().GetProperties();

            foreach (PropertyInfo m_pifDUserBudgetPlanAccess in m_pifDUserBudgetPlanAccesses)
            {
                object m_objValue = m_pifDUserBudgetPlanAccess.GetValue(Data);
                base.SqlParams.Add(new SqlParameter("@in_" + m_pifDUserBudgetPlanAccess.Name, m_objValue));
            }
        ...
        public override void Select(object dbConnection = null)
        {
            ...
            foreach (PropertyInfo m_pifDUserBudgetPlanAccess in m_pifDUserBudgetPlanAccesses)
            {
                if (IsPrimaryKey(m_pifDUserBudgetPlanAccess))
                {
            ...
            base.Message = (m_objDBConnection.Message == string.Empty ?
                (base.Success ? m_objDBConnection.Message : General.EnumDesc(MessageLib.NotExist)) : m_objDBConnection.Message);
        ...
        public override void Update(bool withTrans, object dbConnection = null)
        {
            ...
            PropertyInfo[] m_pifDUserBudgetPlanAccess = Data.GetType().GetProperties();

            foreach (PropertyInfo m_pifDUserBudgetPlanAcce in m_pifDUserBudgetPlanAccess)
            {
                if (IsPrimaryKey(m_pifDUserBudgetPlanAcce))
                {
                    // Do something, to read from the property:
                    object m_objValue = m_pifDUserBudgetPlanAcce.GetValue(Data);
                    base.SqlParams.Add(new SqlParameter("@in_Key" + m_pifDUserBudgetPlanAcce.Name, m_objValue));
                }
            }
            foreach (PropertyInfo m_pifDUserBudgetPlanAcce in m_pifDUserBudgetPlanAccess)
            {
                object m_objValue = m_pifDUserBudgetPlanAcce.GetValue(Data);
                base.SqlParams.Add(new SqlParameter("@in_" + m_pifDUserBudgetPlanAcce.Name, m_objValue));
            }
            base.SqlParams.Add(new SqlParameter("@out_RowCount", SqlDbType.Int));
            base.SqlParams[base.SqlParams.Count - 1].Direction = ParameterDirection.Output;
        ...
        public override void Delete(bool withTrans, object dbConnection = null)
        {
            ...
            foreach (PropertyInfo m_pifDUserBudgetPlanAcc in m_pifDUserBudgetPlanAccess)
            {
                if (IsPrimaryKey(m_pifDUserBudgetPlanAcc))
                {
                    // Do something, to read from the property:
                    object m_objValue = m_pifDUserBudgetPlanAcc.GetValue(Data);
                    base.SqlParams.Add(new SqlParameter("@in_" + m_pifDUserBudgetPlanAcc.Name, m_objValue));
                }
            }
            base.SqlParams.Add(new SqlParameter("@out_RowCount", SqlDbType.Int));
            base.SqlParams[base.SqlParams.Count - 1].Direction = ParameterDirection.Output;
        ...
        public override void DeleteBC(Dictionary<string, List<object>> filter, bool withTrans, object dbConnection = null)
        {
            ...
            base.SqlParams.Clear();
            PropertyInfo[] m_arrPifMUser = Data.GetType().GetProperties();
