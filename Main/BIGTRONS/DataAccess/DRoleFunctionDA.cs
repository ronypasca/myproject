...
{
    public class DRoleFunctionDA : BaseDataAccess
    {
        #region Public Property

        public DRoleFunction Data { get; set; }

        #endregion

        #region Constructors
        /// <summary>
        /// Default constructor for DRoleFunctionDA class
        /// </summary>
        public DRoleFunctionDA()
        {
            Data = new DRoleFunction();
        }
        #endregion
        ...
        {
            PropertyInfo m_pifDRoleFunction = Data.GetType().GetProperty(fieldName);
            return IsPrimaryKey(m_pifDRoleFunction);
        }
        ...
        public override void Insert(bool withTrans, object dbConnection = null)
        {
            ...
            PropertyInfo[] m_arrPifDRoleFunction = Data.GetType().GetProperties();

            foreach (PropertyInfo m_pifDRoleFunction in m_arrPifDRoleFunction)
            {
                object m_objValue = m_pifDRoleFunction.GetValue(Data);
                base.SqlParams.Add(new SqlParameter("@in_" + m_pifDRoleFunction.Name, m_objValue));
            }
        ...
        public override void Select(object dbConnection = null)
        {
            ...
            foreach (PropertyInfo m_pifDRoleFunction in m_arrPifDRoleFunction)
            {
                if (IsPrimaryKey(m_pifDRoleFunction))
                {
        ...
        public override void Update(bool withTrans, object dbConnection = null)
        {
            ...
            foreach (PropertyInfo m_pifDRoleFunction in m_arrPifDRoleFunction)
            {
                if (IsPrimaryKey(m_pifDRoleFunction))
                {
                    // Do something, to read from the property:
                    object m_objValue = m_pifDRoleFunction.GetValue(Data);
                    base.SqlParams.Add(new SqlParameter("@in_Key" + m_pifDRoleFunction.Name, m_objValue));
                }
            }
            foreach (PropertyInfo m_pifDRoleFunction in m_arrPifDRoleFunction)
            {
