...
namespace com.SML.BIGTRONS.Controllers
{
    public class ItemCatalogController : BaseController
    {
        ...
        public ActionResult Add(string Caller)
        {
            ...
            if (string.IsNullOrEmpty(m_objUserVM.VendorID))
            {
                Global.ShowErrorAlert(title, General.EnumDesc(MessageLib.NotAuthorized) + ". User logged didn't register as a Vendor");
                return this.Direct();
            }
            ...
            m_vddItem.Add(UserVM.Prop.VendorID.Name, m_objUserVM.VendorID);
            m_vddItem.Add(UserVM.Prop.VendorDesc.Name, m_objUserVM.VendorDesc);
        ...
        public ActionResult Detail(string Caller, string Selected, string ItemID)
        {
            ...
            m_objItemVM.ListItemPriceVM = new List<ItemPriceVM>();
            if (m_objItemVM != null)
                if (m_objItemVM.HasPrice) m_objItemVM.ListItemPriceVM = GetListItemPrice(m_objItemVM.ItemID, m_objUserVM.VendorID, ref m_strMessage);
            if (m_strMessage != string.Empty)
            {
                Global.ShowErrorAlert(title, m_strMessage);
                return this.Direct();
            ...
            m_objItemVM.ListItemDetailVM = new List<ItemDetailVM>();
            if (m_objItemVM != null)
                m_objItemVM.ListItemDetailVM = GetListItemDetails(m_objItemVM.ItemID, m_objUserVM.VendorID, ref m_strMessage);

            m_objItemVM.ListItemImage = new List<ItemShowCaseVM>();
            if (m_objItemVM != null)
                m_objItemVM.ListItemImage = GetListItemImages(m_objItemVM.ItemID, m_objUserVM.VendorID, ref m_strMessage);


            List<ItemDetailTypeVM> m_lsItemDetailType = GetListItemDetailType(ref m_strMessage);

            PropertyGridParameterCollection m_propGridColl = new PropertyGridParameterCollection();
            if (m_lsItemDetailType.Any())
                m_propGridColl.AddRange(m_lsItemDetailType.Where(d => d.ItemDetailTypeID != "DEC").Select(p => new PropertyGridParameter { Name = p.ItemDetailTypeID, DisplayName = p.ItemDetailTypeDesc, EditorType = PropertyGridEditorType.String }).ToList());

            ViewDataDictionary m_vddItem = new ViewDataDictionary();
            m_vddItem.Add("PropGridCollection", m_propGridColl);
            m_vddItem.Add(UserVM.Prop.VendorID.Name, m_objUserVM.VendorID);
            m_vddItem.Add(UserVM.Prop.VendorDesc.Name, m_objUserVM.VendorDesc);
            ...
            string ControlHasPrice, string ValueItemID = "", string RegionID = "", string ProjectID = "", string ClusterID = "", string UnitTypeID = "", string FilterItemID = "", string FilterItemDesc = "")
        {
            ...
            return new XMVC.PartialViewResult
            {
                ...
                WrapByScriptTag = false,
                ViewData = m_vddItem,
                ViewName = "../ItemCatalog/ItemPrice/VendorPeriod/_Browse"
            };
        }
        [ValidateInput(false)]
        public ActionResult Save(string Action)
        {
            ...
            m_objDBConnection = m_objDItemDetailsDA.BeginTrans(m_strTransName);
            string m_strMessage = string.Empty;
            ...
            try
            {
                ...
                string m_strListItemDetailsVM = this.Request.Params["ListItemDetailVM"];
                ...
                List<ItemDetailVM> m_lstItemDetailVM = JSON.Deserialize<List<ItemDetailVM>>(m_strListItemDetailsVM);
                ...
                m_objMItemVM.HasPrice = bool.Parse(m_strHasPrice??"true");
                ...
                if (m_lstMessage.Count <= 0)
                {
                    ...
                    if (m_lstItemDetailVM.Any()) {
                        ...
                        if (Action == General.EnumDesc(Buttons.ButtonUpdate))
                        {
                            ...
                            m_lstFilter = new List<object>();
                            m_lstFilter.Add(Operator.Equals);
                            m_lstFilter.Add(m_userVM.VendorID);
                        ...
                        foreach (ItemDetailVM objItemDetail in m_lstItemDetailVM)
                        {
                            DItemDetails m_objDItemDetails = new DItemDetails();
                            m_objDItemDetails.ItemDetailID = Guid.NewGuid().ToString().Replace("-", ""); ;
                            m_objDItemDetails.ItemID = m_strItemID;
                            m_objDItemDetails.VendorID = m_userVM.VendorID;
                            m_objDItemDetails.ItemDetailDesc = objItemDetail.ItemDetailDesc;
                            m_objDItemDetails.ItemDetailTypeID = objItemDetail.ItemDetailTypeID;
                            ...
                            if (!m_objDItemDetailsDA.Success || m_objDItemDetailsDA.Message != string.Empty)
                            {
                                ...
                                return this.Direct(false, m_objDItemDetailsDA.Message);
                            }
                        }
                    }
                    #endregion
                    ...
                    if (!m_objDItemDetailsDA.Success && m_objDItemDetailsDA.Message != string.Empty)
                        ...
                        m_objDItemDetailsDA.EndTrans(ref m_objDBConnection, true, m_strTransName);
                }
            }
            catch (Exception ex)
            {
        ...
        public ActionResult DetailItemPrice(string Caller, string Selected)
        {
            ...
            {
                m_objItemPriceVM.ListItemPriceVendorVM = GetListItemPriceVendor(m_objItemPriceVM.ItemPriceID, m_objUserVM.VendorID);
                if (m_objItemPriceVM.ListItemPriceVendorVM == null)
                    m_objItemPriceVM.ListItemPriceVendorVM = new List<ItemPriceVendorVM>();

                m_objItemPriceVM.ListItemPriceVendorPeriodVM = GetListItemPriceVendorPeriod(m_objItemPriceVM.ItemPriceID, m_objUserVM.VendorID);
        ...
        private List<ItemPriceVM> GetListItemPrice(string ItemID,string VendorID, ref string message, string RegionID = null, string ProjectID = null, string ClusterID = null, string UnitTypeID = null)
        {
            ...
            if (m_objDItemPriceDA.Success && m_objDItemPriceDA.Message == string.Empty)
            {
                ...
                select new ItemPriceVM()
                {
                    ...
                    ListItemPriceVendorVM = GetListItemPriceVendor(m_drDItemPriceDA[ItemPriceVM.Prop.ItemPriceID.Name].ToString(),VendorID),
                    ListItemPriceVendorPeriodVM = GetListItemPriceVendorPeriod(m_drDItemPriceDA[ItemPriceVM.Prop.ItemPriceID.Name].ToString(), VendorID)
                }).Distinct().ToList();
            }
            else
        ...
        private List<ItemDetailTypeVM> GetListItemDetailType(ref string message)
        {

            List<ItemDetailTypeVM> m_lstItemDetailTypeVM = new List<ItemDetailTypeVM>();
            MItemDetailTypesDA m_objMItemDetailTypesDA = new MItemDetailTypesDA();
            m_objMItemDetailTypesDA.ConnectionStringName = Global.ConnStrConfigName;

            List<string> m_lstSelect = new List<string>();
            m_lstSelect.Add(ItemDetailTypeVM.Prop.ItemDetailTypeID.MapAlias);
            m_lstSelect.Add(ItemDetailTypeVM.Prop.ItemDetailTypeDesc.MapAlias);
            ...
            //Dictionary<string, OrderDirection> m_objOrderBy = new Dictionary<string, OrderDirection>();
            //m_objOrderBy.Add(ItemDetailTypeVM.Prop.ItemDetailTypeID.Map, OrderDirection.Ascending);

            Dictionary<int, DataSet> m_dicMItemDetailTypesDA = m_objMItemDetailTypesDA.SelectBC(0, null, false, m_lstSelect, null, null, null, null);
            if (m_objMItemDetailTypesDA.Message == string.Empty)
            {
                m_lstItemDetailTypeVM = (
                from DataRow m_drMItemDetailTypes in m_dicMItemDetailTypesDA[0].Tables[0].Rows
                select new ItemDetailTypeVM()
                {
                    ItemDetailTypeID = m_drMItemDetailTypes[ItemDetailTypeVM.Prop.ItemDetailTypeID.Name].ToString(),
                    ItemDetailTypeDesc = m_drMItemDetailTypes[ItemDetailTypeVM.Prop.ItemDetailTypeDesc.Name].ToString()
                }).Distinct().ToList();
            }
            else
                message = m_objMItemDetailTypesDA.Message;

            return m_lstItemDetailTypeVM;

        }
        private List<ItemShowCaseVM> GetListItemImages(string ItemID, string VendorID, ref string message)
        {
            ...
            if (m_objMItemShowCaseDA.Message == string.Empty)
            {
                ...
                message = m_objMItemShowCaseDA.Message;

            return m_lstItemShowCaseVM;
        ...
        public ActionResult GetTemplateWithData(string Caller)
        {
            ...
            List<ItemPriceVendorPeriodVM> m_objLstItemPrice = GetListItemPriceVendorPeriod(string.Empty,m_objUser.VendorID,DateTime.Now.ToString("yyyy-MM-dd"));
            List<ItemDetailVM> m_objLstItemSpec = GetListItemDetails(string.Empty, m_objUser.VendorID, ref m_strMessage);
                ...
                for (int i = sheetDetail.FirstRowNum + 3; i < rowCount; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (!m_hdrInfo.Contains(m_cellValue == null ? string.Empty : m_cellValue.ToString()) && !string.IsNullOrEmpty(row.GetCell(1).ToString()))
                        {
                            ...
                            {
                                var propertyInfo = m_objItemDetailVM.GetType().GetProperty(strItemDetailType.ToString());
                                m_cellValue = row.GetCell(m_dicHeader.Where(x => x.Key.Trim().Equals(strItemDetailType.ToString(), StringComparison.CurrentCultureIgnoreCase)).FirstOrDefault().Value);
        ...
        public ActionResult ReadFileUploadBckup()
        {
            XSSFWorkbook xhssfwb = new XSSFWorkbook();
            HSSFWorkbook hssfwb = new HSSFWorkbook();
            MItemDA m_objMItemDA = new MItemDA();
            ...
            //m_objDBConnection = m_objDItemPriceDA.BeginTrans(m_strTransName);
            //string m_strMessage = string.Empty;
            string m_vendorID = getCurentUser().VendorID;

            try
            {
                SWeb.HttpPostedFile m_fileUpload = X.GetCmp<FileUploadField>(ItemPriceVM.Prop.FileUpload.Name).PostedFile;
                ISheet sheet, sheetItem;
                string filename = Path.GetFileName(Server.MapPath(m_fileUpload.FileName));
                var fileExt = Path.GetExtension(filename);
                if (fileExt == ".xls")
                {
                ...
                for (int i = sheet.FirstRowNum + 3; i < rowCount; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (!m_headerInfo.Contains(m_cellValue == null ? string.Empty : m_cellValue.ToString()) && !string.IsNullOrEmpty(row.GetCell(1).ToString()))
                        {
                            ...
                            //PriceTypeID
                            m_cellValue = row.GetCell(m_dicHeader.Where(x => x.Key.Trim().Equals(ItemPriceVM.Prop.PriceTypeID.Name, StringComparison.CurrentCultureIgnoreCase)).FirstOrDefault().Value);
                            m_objItemPriceVM.PriceTypeID = "MOU";// m_cellValue == null ? string.Empty : m_cellValue.ToString();


                            //Get ItemPriceID if exist
                            string m_strMessage = string.Empty;
                            ItemPriceVM m_objItemPrice = GetItemPrice(m_objItemPriceVM.ItemID, ref m_strMessage, m_objItemPriceVM.RegionID, m_objItemPriceVM.ProjectID, m_objItemPriceVM.ClusterID, m_objItemPriceVM.UnitTypeID);
                            if (m_strMessage == string.Empty)
                            {
                                m_objItemPriceVM.ItemPriceID = m_objItemPrice.ItemPriceID;
                                //m_objItemPriceVM.ItemID = m_listItemPriceVM.FirstOrDefault().ItemID;
                            ...
                            //VendorID
                            m_cellValue = row.GetCell(m_dicHeader.Where(x => x.Key.Trim().Equals(ItemPriceVendorVM.Prop.VendorID.Name, StringComparison.CurrentCultureIgnoreCase)).FirstOrDefault().Value);
                            m_objItemPriceVM.VendorID = m_vendorID;// m_cellValue == null ? string.Empty : m_cellValue.ToString();
                            ...
                            {
                                m_objItemPriceVM.CurrencyID = "IDR";// m_cellValue.ToString();
                            }
                             ...
                             d.UnitTypeID == m_objItemPriceVM.UnitTypeID))
                            {
                                ...
                                //MItem m_objMItem = new MItem();
                                //m_objMItem.ItemID = m_objItemPriceVM.ItemID;
                                //m_objMItemDA.Data = m_objMItem;
                                //m_objMItemDA.Select();
                                //if (m_objMItemDA.Message == string.Empty)
                                //{
                                #region Insert Into DItemPrice
                                //DItemPrice m_objDItemPrice = new DItemPrice();

                                //m_objDItemPrice.ItemPriceID = m_objItemPriceVM.ItemPriceID;

                                //m_objDItemPriceDA.Data = m_objDItemPrice;
                                ////m_objDItemPriceDA.Select();

                                //m_objDItemPrice.PriceTypeID = m_objItemPriceVM.PriceTypeID;
                                //m_objDItemPrice.ProjectID = m_objItemPriceVM.ProjectID;
                                //m_objDItemPrice.RegionID = m_objItemPriceVM.RegionID;
                                //m_objDItemPrice.UnitTypeID = m_objItemPriceVM.UnitTypeID;
                                //m_objDItemPrice.ClusterID = m_objItemPriceVM.ClusterID;

                                //m_objDItemPrice.ItemID = m_objItemPriceVM.ItemID;
                                //m_objDItemPrice.ItemPriceID = Guid.NewGuid().ToString().Replace("-", "");
                                //m_objDItemPriceDA.InsertTemp(true, m_objDBConnection);
                                ...
                                //m_objDItemPriceVendor.IsDefault = m_objItemPriceVM.IsDefault;
                                //m_objDItemPriceVendor.ItemPriceID = m_strItemPriceID;
                                //m_objDItemPriceVendor.VendorID = m_objItemPriceVM.VendorID;
                                //m_objDItemPriceVendor.ItemPriceID = m_strItemPriceID;

                                //m_objDItemPriceVendorDA.Data = m_objDItemPriceVendor;
                                //m_objDItemPriceVendorDA.InsertTemp(true, m_objDBConnection);
                                ...
                                //m_objDItemPriceVendorPeriod.VendorID = m_objItemPriceVM.VendorID;
                                //m_objDItemPriceVendorPeriod.ValidTo = m_objItemPriceVM.ValidTo.Value;
                                //m_objDItemPriceVendorPeriod.ValidFrom = m_objItemPriceVM.ValidFrom.Value;
                                //m_objDItemPriceVendorPeriod.CurrencyID = m_objItemPriceVM.CurrencyID;
                                //m_objDItemPriceVendorPeriod.Amount = m_objItemPriceVM.Amount;


                                //m_objDItemPriceVendorPeriodDA.Data = m_objDItemPriceVendorPeriod;
                                //m_objDItemPriceVendorPeriodDA.InsertTemp(true, m_objDBConnection);
                ...
                if (m_lstMessage.Count <= 0)
                {
                    ...
                    return this.Direct(new { dataItem = new List<ItemVM>(), dataItemPrice = m_lstItemPriceVM });
                }
                else
                {
                    Global.ShowErrorAlert(title, String.Join(Global.NewLineSeparated, m_lstMessage));
        ...
        public ActionResult ReadImageUpload()
        {
            UserVM m_objUserVM = getCurentUser();
            string ItemID = this.Request.Params[ItemVM.Prop.ItemID.Name].ToString();
            List<byte[]> lstByte = new List<byte[]>();
            string ContentType = "";
            string FileName = "";
            string StringRawData = "";
            SWeb.Helpers.WebImage webImage = null;

            List<string> m_lstMessage = new List<string>();
            //byte[] RawData = null;
            for (int x = 0; x < Request.Files.Count; x++)
            {
                FileName = Path.GetFileName(Server.MapPath(Request.Files[x].FileName));
                ContentType = Path.GetExtension(FileName);
                if (Request.Files[x].ContentLength > 500000)
                {
                    webImage = new SWeb.Helpers.WebImage(Request.Files[x].InputStream);
                    webImage.Resize(400, 400, false, true);
                    StringRawData = GetBase64String(webImage.GetBytes());
                }
                else
                {
                   webImage = new SWeb.Helpers.WebImage(Request.Files[x].InputStream);
                ...
                ItemShowCaseVM m_objItemShowCaseVM = new ItemShowCaseVM()
                {
                    Filename = FileName,
                    ContentType = ContentType,
                    RawData = StringRawData,
                    ItemID = ItemID,
                    VendorID = m_objUserVM.VendorID

                };

                //save to MItemShowCase
                MItemShowCase m_objMItemShowCase = new MItemShowCase();
                MItemShowCaseDA m_objMItemShowCaseDA = new MItemShowCaseDA();
                m_objMItemShowCaseDA.ConnectionStringName = Global.ConnStrConfigName;

                m_objMItemShowCase.ItemID = ItemID;
                m_objMItemShowCase.VendorID = m_objUserVM.VendorID;
                m_objMItemShowCase.RawData = StringRawData;
                m_objMItemShowCase.ShowCaseID = Guid.NewGuid().ToString().Replace("-", "");
                m_objMItemShowCase.Filename = FileName;
                m_objMItemShowCase.ContentType = ContentType;
                m_objMItemShowCaseDA.Data = m_objMItemShowCase;
                m_objMItemShowCaseDA.Insert(false);


                if (m_objMItemShowCaseDA.Message != string.Empty)
                    m_lstMessage.Add(m_objMItemShowCaseDA.Message);

                this.GetCmp<Store>("soImage").Add(m_objMItemShowCase);
            }

            if (m_lstMessage.Any())
                Global.ShowErrorAlert("Upload", General.EnumDesc(MessageLib.ErrorSave));
            else
                Global.ShowInfoAlert("Upload", General.EnumDesc(MessageLib.Saved));
            
            return this.Direct(true);
        }

        [ValidateInput(false)]
        public ActionResult SaveUploadBckup(string Action)
        {
            //if (!(Action == General.EnumDesc(Buttons.ButtonAdd) ? HasAccess(General.GetVariableName(() => new HasAccessVM().Add))
            //    : HasAccess(General.GetVariableName(() => new HasAccessVM().Update))))
            //    return this.Direct(false, General.EnumDesc(MessageLib.NotAuthorized));
            ...
            try
            {
                ...
                if (m_lstMessage.Count <= 0)
                {
                    ...
                    if (m_lstMessage.Count <= 0)
                    {
                        ...
                        m_objMTask.TaskDesc = $@"Upload Item - {m_userLogged.VendorDesc}";
                        m_objMTask.TaskDateTimeStamp = DateTime.Now;
                        m_objMTask.TaskOwnerID = RoleIDUser;// GetFirstApprovalRole(General.EnumDesc(TaskType.UploadItem), ref messageErr);// "PWR";//GetCurrentApproval(TaskType.ScheduleInvitation.ToString(),0);//TODO  get first level approval ownerid
                        m_objMTask.StatusID = (int)TaskStatus.InProgress;
                        m_objMTask.CurrentApprovalLvl = 0;
                        m_objMTask.Remarks = "New Upload by Vendor";
                        m_objMTask.Summary = $@"Upload By : { m_userLogged.VendorDesc } ({m_userLogged.VendorID}). Total : {m_lstItemPriceVM.Count} Item(s)";
                    ...
                    if (m_objMTaskDA.Success && m_lstMessage.Count <= 0)
                    {
                        ...
                        m_objMTaskDA.EndTrans(ref m_objDBConnection, false, m_strTransName);
                    ...
                    foreach (ItemPriceVM objItemPriceVM in m_lstItemPriceVM)
                    {
                        ...
                        m_objDItemPriceUpload.ValidFrom = objItemPriceVM.ValidFrom.Value;
                        m_objDItemPriceUpload.ValidTo = objItemPriceVM.ValidTo.Value;
                        ...
                        m_objDItemPriceUploadDA.Insert(true, m_objDBConnection);

                        if (!m_objDItemPriceUploadDA.Success || m_objDItemPriceUploadDA.Message != string.Empty)
                        {
                            m_objDItemPriceUploadDA.EndTrans(ref m_objDBConnection, false, m_strTransName);
                        ...
                        //    {
                        //        //insert new price
                        //        m_objDItemPriceVendorPeriod.TaskID = m_objMTaskDA.Data.TaskID;
                        //        m_objDItemPriceVendorPeriod.Visible = false;
                        //        m_objDItemPriceVendorPeriod.ItemPriceID = m_strItemPriceID;
                        ...
                        //        ItemPriceVendorPeriodVM m_objdItemPriceVendorPeriodVM = m_lstdItemPriceVendorPeriodVM.Where(d => d.ValidFrom > objItemPriceVM.ValidFrom.Value).FirstOrDefault();
                        //        m_objDItemPriceVendorPeriod = new DItemPriceVendorPeriod();
                        //        if (m_objdItemPriceVendorPeriodVM != null)
                        //        {
            ...
            m_objDItemPriceUploadDA.EndTrans(ref m_objDBConnection, true, m_strTransName);
            if (m_lstMessage.Count <= 0)
            {
                Global.ShowInfoAlert(title, General.EnumDesc(Action == General.EnumDesc(Buttons.ButtonAdd) ? MessageLib.Added : MessageLib.Updated));
                return this.Direct(true);
        ...
        public ActionResult SaveUpload(string Action)
        {
            ...
            try
            {
                ...
                List<ItemVM> m_lstItemVM = string.IsNullOrEmpty(m_strListItemVM) ? new List<ItemVM>() : JSON.Deserialize<List<ItemVM>>(m_strListItemVM);
                ...
                if (m_lstMessage.Count <= 0)
                {
                    ...
                    if (m_lstMessage.Count <= 0)
                    {
                        ...
                        m_objMTask.TaskDesc = $@"Upload Item - {m_userLogged.VendorDesc}";
                        m_objMTask.TaskDateTimeStamp = DateTime.Now;
                        m_objMTask.TaskOwnerID = GetCurrentApproval(General.EnumDesc(TaskType.UploadItem), 1);// GetFirstApprovalRole(General.EnumDesc(TaskType.UploadItem), ref messageErr);// "PWR";//GetCurrentApproval(TaskType.ScheduleInvitation.ToString(),0);//TODO  get first level approval ownerid
                        m_objMTask.StatusID = (int)TaskStatus.InProgress;
                        m_objMTask.CurrentApprovalLvl = 1; // jump to catalog Admin Role
                        m_objMTask.Remarks = "New Upload by Vendor";
                        m_objMTask.Summary = $@"Upload By : { m_userLogged.VendorDesc } ({m_userLogged.VendorID}). Total : {m_lstItemPriceVM.Count} Item(s)";
        ...
        public ActionResult DeleteItemImage(string Selected) {
            //if (!HasAccess(General.GetVariableName(() => new HasAccessVM().Delete)))
            //    return this.Direct(false, General.EnumDesc(MessageLib.NotAuthorized));

            List<ItemShowCaseVM> m_lstSelectedRow = new List<ItemShowCaseVM>();
            m_lstSelectedRow = JSON.Deserialize<List<ItemShowCaseVM>>(Selected);
            ...
            try
            {
                foreach (ItemShowCaseVM m_objItemShowCaseVM in m_lstSelectedRow)
                {
                    List<string> m_lstKey = new List<string>();
                    Dictionary<string, List<object>> m_objFilter = new Dictionary<string, List<object>>();
                    PropertyInfo[] m_arrPifItemVM = m_objItemShowCaseVM.GetType().GetProperties();

                    foreach (PropertyInfo m_pifItemVM in m_arrPifItemVM)
                    {
                        string m_strFieldName = m_pifItemVM.Name;
                        object m_objFieldValue = m_pifItemVM.GetValue(m_objItemShowCaseVM);
                        if (m_objItemShowCaseVM.IsKey(m_strFieldName))
                        {

                            MItemShowCase m_objMItemShowCase = new MItemShowCase();
                            m_objMItemShowCase.ShowCaseID = m_objFieldValue.ToString();
                            m_objMItemShowCaseDA.Data = m_objMItemShowCase;
                            m_objMItemShowCaseDA.Select();

                            m_objMItemShowCaseDA.Delete(false);
                        }
                        else break;
                    }

                    if (m_objMItemShowCaseDA.Message != string.Empty)
                        m_lstMessage.Add("[" + string.Join(Global.OneLineSeparated, m_lstKey) + "] " + m_objMItemShowCaseDA.Message);
                }
            }
            catch (Exception ex)
            {
