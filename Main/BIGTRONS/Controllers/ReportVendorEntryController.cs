...
{
    public class ReportVendorEntryController : BaseController
    {
        ...
        {
            base.Initialize();
            return View("~/Views/Report/VendorEntry/Index.cshtml");
        }
        ...
        public ActionResult List()
        {
            ...
            return new XMVC.PartialViewResult
            {
                ...
                ViewName = "~/Views/Report/VendorEntry/_List.cshtml",
        ...
        public ActionResult Read(StoreRequestParameters parameters)
        {
            ...
            m_lstFilter.Add("");
            m_objFilter.Add(BudgetPlanVersionVendorVM.Prop.PackageID.Map, m_lstFilter);
        ...
        public ActionResult ExportReportToExcel(string BPID, string VendorID)
        {
            string message = "";
            if (string.IsNullOrEmpty(VendorID))
            {
                VendorID = getCurentUser().VendorID;
            }
            ...
            {                
                ...
                if (m_objDPackageDA.Message == string.Empty && m_objDPackageDA.AffectedRows>0)
                {
                    ...
                    m_lstSelect.Add(PackageListVM.Prop.BudgetPlanVersionVendorID.MapAlias);
                    m_lstSelect.Add(PackageListVM.Prop.FeePercentageVendorEntry.MapAlias);
                    ...
                    m_objFilter.Add(PackageListVM.Prop.VendorID.Map, m_lstFilter);


                    m_dicSPackageDA = m_objDPackageDA.SelectBCforReportVendorEntry(0, null, false, m_lstSelect, m_objFilter, null, null, null, null);
                    if (m_objDPackageDA.Message == string.Empty && m_objDPackageDA.AffectedRows > 0)
                    {
                        m_lstPackageVM = (
                        from DataRow m_drSPackageDA in m_dicSPackageDA[0].Tables[0].Rows
                            select new PackageListVM()
                            {
                                ...
                                UnitTypeDesc = m_drSPackageDA[PackageListVM.Prop.UnitTypeDesc.Name].ToString(),
                                Area = (decimal)m_drSPackageDA[PackageListVM.Prop.LastApprovedVersionArea.Name],
                                FeePercentage = (decimal)m_drSPackageDA[PackageListVM.Prop.FeePercentageVendorEntry.Name],
                                BlokNo = m_drSPackageDA[PackageListVM.Prop.LastApprovedVersionBlockNo.Name].ToString(),
                                Unit = (decimal)m_drSPackageDA[PackageListVM.Prop.LastApprovedVersionUnit.Name],
                                BudgetPlanVersionVendorID = m_drSPackageDA[PackageListVM.Prop.BudgetPlanVersionVendorID.Name].ToString()
                            }
                        ).ToList();
                    }
                }
                    ...
                    {                       
                        switch (x)
                        {
                            ...
                            case 5:
                                ...
                                break;
                            case 6:
                                HargaPerUnit = GetVendorSubtotal(objPackageList.BudgetPlanVersionVendorID, objPackageList.FeePercentage,ref message);
                                HargaPerMeter = HargaPerUnit / objPackageList.Area;

                                //HargaPerUnit = Math.Round(HargaPerMeter, 0);
                                arrayRow[x] = HargaPerMeter.ToString("#,##0.00");
                                break;
                            case 7:
                                //HargaPerMeter = Math.Round(HargaPerUnit, 0);
