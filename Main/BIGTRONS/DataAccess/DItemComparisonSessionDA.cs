...
{
    public class DItemComparisonSessionDA : BaseDataAccess
    {
        #region Public Property

        public DItemComparisonSession Data { get; set; }

        #endregion

        #region Constructors
        /// <summary>
        /// Default constructor for DItemComparisonDetailsDA class
        /// </summary>
        public DItemComparisonSessionDA()
        {
            Data = new DItemComparisonSession();
        }
        #endregion
        ...
        {
            PropertyInfo m_pifDItemComparisonDetails = Data.GetType().GetProperty(fieldName);
            return IsPrimaryKey(m_pifDItemComparisonDetails);
        }
        ...
        public override void Insert(bool withTrans, object dbConnection = null)
        {
            ...
            PropertyInfo[] m_arrPifDItemComparisonSession = Data.GetType().GetProperties();

            foreach (PropertyInfo m_pifDItemComparisonSession in m_arrPifDItemComparisonSession)
            {
                object m_objValue = m_pifDItemComparisonSession.GetValue(Data);
                base.SqlParams.Add(new SqlParameter("@in_" + m_pifDItemComparisonSession.Name, m_objValue));
            }
        ...
        public override void Select(object dbConnection = null)
        {
            ...
            foreach (PropertyInfo m_pifDItemComparisonSession in m_arrPifDItemComparisonSession)
            {
                if (IsPrimaryKey(m_pifDItemComparisonSession))
                {
        ...
        public override void Update(bool withTrans, object dbConnection = null)
        {
            ...
            foreach (PropertyInfo m_pifDItemComparisonSession in m_arrPifDItemComparisonSession)
            {
                if (IsPrimaryKey(m_pifDItemComparisonSession))
                {
                    // Do something, to read from the property:
                    object m_objValue = m_pifDItemComparisonSession.GetValue(Data);
                    base.SqlParams.Add(new SqlParameter("@in_Key" + m_pifDItemComparisonSession.Name, m_objValue));
                }
            }
            foreach (PropertyInfo m_pifDItemComparisonSession in m_arrPifDItemComparisonSession)
            {
