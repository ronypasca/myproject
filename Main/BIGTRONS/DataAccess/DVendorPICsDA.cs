...
{
    public class DVendorPICsDA : BaseDataAccess
    {
        #region Public Property

        public DVendorPICs Data { get; set; }

        #endregion

        #region Constructors
        /// <summary>
        /// Default constructor for DVendorPICsDA class
        /// </summary>
        public DVendorPICsDA()
        {
            Data = new DVendorPICs();
        }
        #endregion
        ...
        {
            PropertyInfo m_pifDVendorPICs = Data.GetType().GetProperty(fieldName);
            return IsPrimaryKey(m_pifDVendorPICs);
        }
        ...
        public override void Insert(bool withTrans, object dbConnection = null)
        {
            ...
            PropertyInfo[] m_arrPifDVendorPICs = Data.GetType().GetProperties();

            foreach (PropertyInfo m_pifDVendorPICs in m_arrPifDVendorPICs)
            {
                object m_objValue = m_pifDVendorPICs.GetValue(Data);
                base.SqlParams.Add(new SqlParameter("@in_" + m_pifDVendorPICs.Name, m_objValue));
            }
        ...
        public override void Select(object dbConnection = null)
        {
            ...
            foreach (PropertyInfo m_pifDVendorPICs in m_arrPifDVendorPICs)
            {
                if (IsPrimaryKey(m_pifDVendorPICs))
                {
        ...
        public override void Update(bool withTrans, object dbConnection = null)
        {
            ...
            foreach (PropertyInfo m_pifDVendorPICs in m_arrPifDVendorPICs)
            {
                if (IsPrimaryKey(m_pifDVendorPICs))
                {
                    // Do something, to read from the property:
                    object m_objValue = m_pifDVendorPICs.GetValue(Data);
                    base.SqlParams.Add(new SqlParameter("@in_Key" + m_pifDVendorPICs.Name, m_objValue));
                }
            }
            foreach (PropertyInfo m_pifDVendorPICs in m_arrPifDVendorPICs)
            {
