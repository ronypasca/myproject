...
{
    public class BudgetPlanVersionMutualController : BaseController
    {
        ...
        public ActionResult Read(StoreRequestParameters parameters)
        {
            ...
            if (m_intCount > 0)
            {
                ...
                m_lstFilter = new List<object>();
                m_lstFilter.Add(Operator.Equals);
                m_lstFilter.Add(BudgetPlanPeriod.FQ);
                ...
                if (m_objDBudgetPlanVersionVendorDA.Message == string.Empty)
                {
                    m_lstBudgetPlanVersionVendorVM = (
                        ...
                        select new BudgetPlanVersionVendorVM()
                        {
                            ...
                            //ListBudgetPlanVersionAssignmentVM = GetListBudgetPlanVersionVendor(m_drMBudgetPlanVersionVendorDA[BudgetPlanVersionVendorVM.Prop.BudgetPlanID.Name].ToString(), m_drMBudgetPlanVersionVendorDA[BudgetPlanVersionVendorVM.Prop.BudgetPlanVersion.Name].ToString(), ref messages)
                        }
                    ).ToList();
        ...
        public ActionResult Save(string Action)
        {
            ...
            DBudgetPlanVersionVendorDA m_vrsDa = new DBudgetPlanVersionVendorDA();
            m_vrsDa.ConnectionStringName = Global.ConnStrConfigName;

            DBudgetPlanVersionPeriodDA m_objDBudgetPlanVersionPeriod = new DBudgetPlanVersionPeriodDA();
            m_objDBudgetPlanVersionPeriod.ConnectionStringName = Global.ConnStrConfigName;



            string m_strTransactionName = "BudgetPlanEntry_Trans";
            object m_objDBConnection = m_objDBudgetPlanVersionMutualDA.BeginTrans(m_strTransactionName);
            try
            {
                string m_strBudgetPlanID = this.Request.Params[BudgetPlanVersionMutualVM.Prop.BudgetPlanID.Name];
                string m_strBudgetPlanVersionVendorID = this.Request.Params[BudgetPlanVersionMutualVM.Prop.BudgetPlanVersionVendorID.Name];
                decimal m_decFeePercentage = this.Request.Params[BudgetPlanVersionMutualVM.Prop.FeePercentage.Name].ToString().Length == 0 ? 0 : decimal.Parse(this.Request.Params[BudgetPlanVersionMutualVM.Prop.FeePercentage.Name].ToString());
                string m_strBudgetPlanVersionPeriodID = this.Request.Params[BudgetPlanVersionMutualVM.Prop.BudgetPlanVersionPeriodID.Name];
                int m_intBudgetPlanVersion = int.Parse(this.Request.Params[BudgetPlanVersionMutualVM.Prop.BudgetPlanVersion.Name]);
                //Get Period ID versi IQ
                List<string> m_lstSelect = new List<string>();
                ...
                //Dictionary<int, DataSet> m_dicDBudgetPlanPeriod = m_objDBudgetPlanVersionPeriod.SelectBC(0, 1, false, m_lstSelect, m_objFilter, null, null, null);
                //if (m_objDBudgetPlanVersionPeriod.Success)
                //{
                //    foreach (DataRow m_drDBudgetPlanPeriodDA in m_dicDBudgetPlanPeriod[0].Tables[0].Rows)
                //    {
                //        m_strBudgetPlanVersionPeriodID = m_drDBudgetPlanPeriodDA[BudgetPlanVersionPeriodVM.Prop.BudgetPlanVersionPeriodID.Name].ToString();
                //    }
                //}
                //else
                //    m_lstMessage.Add(m_objDBudgetPlanVersionPeriod.Message);

                //Dictionary<string, object>[] m_arrListVendor = JSON.Deserialize<Dictionary<string, object>[]>(this.Request.Params["listVendor"]);
                ...
                if (m_lstMessage.Count <= 0)
                {
                    List<DBudgetPlanVersionMutual> m_lstDBudgetPlanVersionMutual = new List<DBudgetPlanVersionMutual>();

                    foreach (Dictionary<string, object> m_dicBudgetPlanVersionEntry in m_arrSctructure) {

                        DBudgetPlanVersionMutual obj = new DBudgetPlanVersionMutual();
                        obj.BudgetPlanVersionStructureID = m_dicBudgetPlanVersionEntry[BudgetPlanVersionMutualVM.Prop.BudgetPlanVersionStructureID.Name.ToLower()].ToString();
                        obj.Info = m_dicBudgetPlanVersionEntry[BudgetPlanVersionMutualVM.Prop.Info.Name.ToLower()].ToString();
                        obj.MaterialAmount = m_dicBudgetPlanVersionEntry[BudgetPlanVersionMutualVM.Prop.MaterialAmount.Name.ToLower()] == null ? 0 :Convert.ToDecimal(m_dicBudgetPlanVersionEntry[BudgetPlanVersionMutualVM.Prop.MaterialAmount.Name.ToLower()].ToString());
                        obj.MiscAmount = m_dicBudgetPlanVersionEntry[BudgetPlanVersionMutualVM.Prop.MiscAmount.Name.ToLower()] == null ? 0 : Convert.ToDecimal(m_dicBudgetPlanVersionEntry[BudgetPlanVersionMutualVM.Prop.MiscAmount.Name.ToLower()].ToString());
                        obj.Volume = m_dicBudgetPlanVersionEntry[BudgetPlanVersionMutualVM.Prop.Volume.Name.ToLower()] == null ? 0 : (Decimal)Convert.ToDouble(m_dicBudgetPlanVersionEntry[BudgetPlanVersionMutualVM.Prop.Volume.Name.ToLower()].ToString());
                        obj.WageAmount = m_dicBudgetPlanVersionEntry[BudgetPlanVersionMutualVM.Prop.WageAmount.Name.ToLower()] == null ? 0 : Convert.ToDecimal(m_dicBudgetPlanVersionEntry[BudgetPlanVersionMutualVM.Prop.WageAmount.Name.ToLower()].ToString());
                        m_lstDBudgetPlanVersionMutual.Add(obj);
                    }

                    foreach (DBudgetPlanVersionMutual item in m_lstDBudgetPlanVersionMutual)
                    {
                        DBudgetPlanVersionMutual send = new DBudgetPlanVersionMutual();
                        send.BudgetPlanVersionStructureID = item.BudgetPlanVersionStructureID;
                        m_objDBudgetPlanVersionMutualDA.Data = send;
                        m_objDBudgetPlanVersionMutualDA.Select();
                        //Create or Update Table Mutual
                        if (m_objDBudgetPlanVersionMutualDA.AffectedRows > 0)
                        {
                            send.Volume = item.Volume;
                            send.MaterialAmount = item.MaterialAmount;
                            send.WageAmount = item.WageAmount;
                            send.MiscAmount = item.MiscAmount;
                            send.MaterialAmount = item.MaterialAmount;
                            send.Info = item.Info;
                        ...
                        else
                        {
                            ...
                            send.MiscAmount = item.MiscAmount;
                            send.MaterialAmount = item.MaterialAmount;
                            send.Info = item.Info==null? "": item.Info; //check again
                            m_objDBudgetPlanVersionMutualDA.Insert(true, m_objDBConnection);
                        }
                        if (!m_objDBudgetPlanVersionMutualDA.Success)
                        {
                            m_lstMessage.Add(send.Info+" " + m_objDBudgetPlanVersionMutualDA.Message);
                            break;
                        }

                        //Update Table Version Entry

                        //Get Vendor ID List from Assigment
                        ...
                        m_lstFilter = new List<object>();
                        m_lstFilter.Add(Operator.Equals);
                        m_lstFilter.Add(item.BudgetPlanVersionStructureID);
                        m_objFilter.Add(BudgetPlanVersionAssignmentVM.Prop.BudgetPlanVersionStructureID.Map, m_lstFilter);

                        Dictionary<int, DataSet> m_dicTBudgetPlanDA = m_objDBudgetPlanVersionAssignmentDA.SelectBC(0, null, false, m_lstSelect, m_objFilter, null, null, null);
                        string VendorID = "";
                        List<string> lst_VendorID = new List<string>();
                        if (m_objDBudgetPlanVersionAssignmentDA.AffectedRows > 0 && m_objDBudgetPlanVersionAssignmentDA.Success)
                        {
                            foreach (DataRow m_drTBudgetPlanDA in m_dicTBudgetPlanDA[0].Tables[0].Rows)
                            {
                                VendorID = m_drTBudgetPlanDA[BudgetPlanVersionAssignmentVM.Prop.VendorID.Name].ToString();
                                lst_VendorID.Add(VendorID);
                                ////Get BudgetPlanVersionPeriodID
                                //DBudgetPlanVersionPeriod VersionPeriod = new DBudgetPlanVersionPeriod();
                                //DBudgetPlanVersionPeriodDA m_prdDA = new DBudgetPlanVersionPeriodDA();
                                //m_prdDA.Data = VersionPeriod;
                                ...
                                //m_lstFilter.Add("FQ");
                                //m_objFilter.Add(BudgetPlanVersionPeriodVM.Prop.BudgetPlanPeriodID.Map, m_lstFilter);
                                //string BudgetPlanVersionPeriodID = "";
                                //Dictionary<int, DataSet> m_dicTBudm_dicPlanPeriod = m_prdDA.SelectBC(0, null, false, m_lstSelect, m_objFilter, null, null, null);
                                //if (m_prdDA.AffectedRows > 0 && m_prdDA.Success)
                                //{
                                //    foreach (DataRow dr_ in m_dicTBudm_dicPlanPeriod[0].Tables[0].Rows)
                                //        BudgetPlanVersionPeriodID = dr_[BudgetPlanVersionPeriodVM.Prop.BudgetPlanVersionPeriodID.Name].ToString();
                                //}

                                //Get BudgetPlanVersionVendorID
                                DBudgetPlanVersionVendor VersionVendor = new DBudgetPlanVersionVendor();
                                m_vrsDa.Data = VersionVendor;
                                m_lstSelect = new List<string>();
                                m_lstSelect.Add(BudgetPlanVersionVendorVM.Prop.BudgetPlanVersionVendorID.MapAlias);
                                ...
                                m_lstFilter = new List<object>();
                                m_lstFilter.Add(Operator.Equals);
                                m_lstFilter.Add(m_strBudgetPlanVersionPeriodID);
                                ...
                                string BudgetPlanVersionVendorID = "";

                                Dictionary<int, DataSet> m_dicTBudm_dicPlanVendor = m_vrsDa.SelectBC(0, null, false, m_lstSelect, m_objFilter, null, null, null);
                                if (m_vrsDa.AffectedRows > 0 && m_vrsDa.Success)
                                {
                                    foreach (DataRow dr_ in m_dicTBudm_dicPlanVendor[0].Tables[0].Rows)
                                    {
                                        BudgetPlanVersionVendorID = dr_[BudgetPlanVersionVendorVM.Prop.BudgetPlanVersionVendorID.Name].ToString();
                                        ...
                                        m_objDBudgetPlanVersionEntry.BudgetPlanVersionVendorID = BudgetPlanVersionVendorID;
                                        m_objDBudgetPlanVersionEntryDA.Data = m_objDBudgetPlanVersionEntry;

                                        m_objDBudgetPlanVersionEntryDA.Select();

                                        if (m_objDBudgetPlanVersionEntryDA.AffectedRows > 0)
                                        {
                        ...
                        else if (m_objDBudgetPlanVersionAssignmentDA.AffectedRows <= 0 && m_objDBudgetPlanVersionAssignmentDA.Success)
                        {   
                            ...
                            if (m_vrsDa.AffectedRows > 0 && m_vrsDa.Success)
                            {
                                ...
                                {
                                    BudgetPlanVersionVendorID = dr_[BudgetPlanVersionVendorVM.Prop.BudgetPlanVersionVendorID.Name].ToString();
                                    string vendorName = dr_[BudgetPlanVersionVendorVM.Prop.VendorDesc.Name].ToString();
                                    //Updating Entry
                                    DBudgetPlanVersionEntry m_objDBudgetPlanVersionEntry = new DBudgetPlanVersionEntry();
                                    m_objDBudgetPlanVersionEntry.BudgetPlanVersionStructureID = item.BudgetPlanVersionStructureID;
                                    m_objDBudgetPlanVersionEntry.BudgetPlanVersionVendorID = BudgetPlanVersionVendorID;
                                    ...
                                    {
                                        m_objDBudgetPlanVersionEntryDA.Message = "Vendor entry didn't found. Please make sure if Vendor "+vendorName+" already submit the vendor entry.";
                                    }
                                }
                            }
                        }
                    ...
                    //Update Table Version Additional
                    if (m_objDBudgetPlanVersionEntryDA.Success)
                    {
            ...
            if (m_lstMessage.Count <= 0)
            {
                m_objDBudgetPlanVersionMutualDA.EndTrans(ref m_objDBConnection, true, m_strTransactionName);
            ...
            m_objDBudgetPlanVersionMutualDA.EndTrans(ref m_objDBConnection, false, m_strTransactionName);
            Global.ShowErrorAlert(title, string.Join(Global.NewLineSeparated, m_lstMessage));
        ...
        public ActionResult Verify(string Selected)
        {
            ...
            try
            {
                ...
                foreach (BudgetPlanVersionVendorVM item in m_lstSelectedRow)
                {
                    ...
                    if (!IsBudgetPlanVersionPeriodIDOpen(item.BudgetPlanVersionPeriodID, GetVendorID(),
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (m_objTBudgetPlanDA.Message == string.Empty)
                                {
                                    ...
                                    m_objBudgetPlanVersionEntryVM.VendorID = GetVendorID();
                                    ...
                                    m_node.Children.AddRange(CheckMyChild(m_objBudgetPlanVersionEntryVM.BudgetPlanID,
                                        ...
                                        item.BudgetPlanVersionVendorID, ref m_lstBudgetPlanIDNotSubmitedYet));
        ...
        private List<BudgetPlanVersionVendorVM> GetListBudgetPlanVersionVendor(string budgetPlanID, string budgetPlanVersion, ref string message)
        {
            List<BudgetPlanVersionVendorVM> m_lstBudgetPlanVersionVendorVM = new List<BudgetPlanVersionVendorVM>();
            DBudgetPlanVersionVendorDA m_objDBudgetPlanVersionVendorDA = new DBudgetPlanVersionVendorDA();
            m_objDBudgetPlanVersionVendorDA.ConnectionStringName = Global.ConnStrConfigName;
        ...
        private List<BudgetPlanPeriodVM> GetBudgetPlanPeriod(string BudgetPlanID, string BudgetPlanVersion)
        {
           // bool m_boolReturn = false;
            ...
            if (m_objDBudgetPlanVersionPeriodDA.Message == string.Empty)
            {
                m_lstBudgetPlanPeriodVM = (
                    from DataRow m_drDBudgetPlanVersionStructureDA in m_dicDBudgetPlanVersionVendorDA[0].Tables[0].Rows
                    select new BudgetPlanPeriodVM()
                    {
                       ...
                       BudgetPlanVersionVendorID = m_drDBudgetPlanVersionStructureDA[BudgetPlanVersionStructureVM.Prop.BudgetPlanVersionVendorID.Name].ToString()

                    }
                ).ToList();
            }

            return m_lstBudgetPlanPeriodVM;
        }
        ...
        private BudgetPlanVersionEntryVM GetSelectedData(Dictionary<string, object> selected, ref string message)
        {
            ...
            if (m_objTBudgetPlanDA.Message == string.Empty)
            {
                ...
                Node m_node = new Node() { NodeID = "root", Expanded = true };
                NodeCollection m_ncStructure = LoadMyChild(m_objBudgetPlanVersionEntryVM.BudgetPlanID, m_objBudgetPlanVersionEntryVM.BudgetPlanVersion, m_objBudgetPlanVersionEntryVM.FilterItemTypeID, m_strBudgetPlanVersionVendorID, m_strVendorID);
            ...
            string ParentItemID = "0", int ParentVersion = 0, int ParentSequence = 0, string SequenceDesc = "")
        {
            ...
            List<BudgetPlanVersionMutualVM> m_lstBudgetPlanVersionMutualVM = new List<BudgetPlanVersionMutualVM>();
            ...
            m_lstBudgetPlanVersionMutualVM = m_lstBudgetPlanVersionMutualVM.OrderBy(m => m.Sequence).ToList();
            for (int i = 0; i < m_lstBudgetPlanVersionMutualVM.Count; i++)
            {
                DBudgetPlanVersionMutualDA m_objDBudgetPlanVersionMutualDA = new DBudgetPlanVersionMutualDA();
                m_objDBudgetPlanVersionMutualDA.ConnectionStringName = Global.ConnStrConfigName;

                m_lstSelect = new List<string>();
                m_lstSelect.Add(BudgetPlanVersionMutualVM.Prop.Info.MapAlias);
                m_lstSelect.Add(BudgetPlanVersionMutualVM.Prop.Volume.MapAlias);
                m_lstSelect.Add(BudgetPlanVersionMutualVM.Prop.MaterialAmount.MapAlias);
                m_lstSelect.Add(BudgetPlanVersionMutualVM.Prop.WageAmount.MapAlias);
                m_lstSelect.Add(BudgetPlanVersionMutualVM.Prop.MiscAmount.MapAlias);

                m_objFilter = new Dictionary<string, List<object>>();               

                m_lstFilter = new List<object>();
                m_lstFilter.Add(Operator.Equals);
                m_lstFilter.Add(m_lstBudgetPlanVersionMutualVM[i].BudgetPlanVersionStructureID);
                m_objFilter.Add(BudgetPlanVersionMutualVM.Prop.BudgetPlanVersionStructureID.Map, m_lstFilter);

                

                Dictionary<int, DataSet> m_dicDBudgetPlanVersionEntryDA = m_objDBudgetPlanVersionMutualDA.SelectBC(0, null, false, m_lstSelect, m_objFilter, null, null, null, null);
                if (m_objDBudgetPlanVersionMutualDA.Message == string.Empty)
                {
                    DataRow m_drDBudgetPlanVersionutualDA = m_dicDBudgetPlanVersionEntryDA[0].Tables[0].Rows[0];
                    m_lstBudgetPlanVersionMutualVM[i].Volume = decimal.Parse(m_drDBudgetPlanVersionutualDA[BudgetPlanVersionMutualVM.Prop.Volume.Name].ToString()) == 0 ? (decimal?)null : decimal.Parse(m_drDBudgetPlanVersionutualDA[BudgetPlanVersionEntryVM.Prop.Volume.Name].ToString());
                    m_lstBudgetPlanVersionMutualVM[i].MaterialAmount = decimal.Parse(m_drDBudgetPlanVersionutualDA[BudgetPlanVersionMutualVM.Prop.MaterialAmount.Name].ToString()) == 0 ? (decimal?)null : decimal.Parse(m_drDBudgetPlanVersionutualDA[BudgetPlanVersionEntryVM.Prop.MaterialAmount.Name].ToString());
                    m_lstBudgetPlanVersionMutualVM[i].WageAmount = decimal.Parse(m_drDBudgetPlanVersionutualDA[BudgetPlanVersionMutualVM.Prop.WageAmount.Name].ToString()) == 0 ? (decimal?)null : decimal.Parse(m_drDBudgetPlanVersionutualDA[BudgetPlanVersionEntryVM.Prop.WageAmount.Name].ToString());
                    m_lstBudgetPlanVersionMutualVM[i].MiscAmount = decimal.Parse(m_drDBudgetPlanVersionutualDA[BudgetPlanVersionMutualVM.Prop.MiscAmount.Name].ToString()) == 0 ? (decimal?)null : decimal.Parse(m_drDBudgetPlanVersionutualDA[BudgetPlanVersionEntryVM.Prop.MiscAmount.Name].ToString());
                    m_lstBudgetPlanVersionMutualVM[i].Info = m_drDBudgetPlanVersionutualDA[BudgetPlanVersionMutualVM.Prop.Info.Name].ToString();
                }
            }

            int m_intSequence = 1;

            foreach (BudgetPlanVersionMutualVM item in m_lstBudgetPlanVersionMutualVM)
            {
                ...
                m_node.Children.AddRange(LoadMyChild(budgetPlanID, budgetPlanVersion, filterItemTypeID, BudgetPlanVersionVendorID, VendorID, item.ItemID, item.Version, item.Sequence, m_strSequenceDesc));
        ...
        private NodeCollection LoadMyChildAdditional(string budgetPlanID, int budgetPlanVersion,
            string ParentItemID = "0", int ParentVersion = 0, int ParentSequence = 0)
        {
            NodeCollection m_nodeColChild = new NodeCollection();
            ...
            {
                Node m_node = new Node();
                m_node.Children.AddRange(LoadMyChildAdditional(budgetPlanID, budgetPlanVersion, item.ItemID, item.Version, item.Sequence));
                m_node.Icon = m_node.Children.Count > 0 ? Icon.Folder : Icon.PageWhite;
                m_node.Expanded = m_node.Children.Count > 0 ? true : false;
                m_node.Leaf = m_node.Children.Count > 0 ? false : true;
